...
function formulaires_signature_verifier_dist($id_article) {
	$erreurs = array();
	$oblis = array('session_nom','session_prenom', 'session_naissance','session_email', 'session_ecole', 'session_pays');
	include_spip('base/abstract_sql');
	...
	if ($naissance = _request('session_naissance') and strlen($naissance)==0) {
		...
		$erreurs['session_naissance'] = _T('form_annee_naissance_format_incorrect');
	}

	if ($ecole_alumni = _request('session_ecole_ajout') and strlen($ecole_alumni) ==0) {
		if ($ecole_alumni = _request('session_ecole') and strlen($ecole_alumni) ==0) {
...
function formulaires_signature_traiter_dist($id_article) {
	...
	if (spip_connect()) {
		...
		if($ecole_alumni = _request('session_ecole_ajout') and strlen($ecole_alumni)==0){
			$ecole_alumni = _request('session_ecole');
		}
		...
		$membre_asso_alumni='inconnu';
		if(_request('membre_asso_alumni_oui')){
			$membre_asso_alumni='oui';
		}

		else if(_request('membre_asso_alumni_non')){
			$membre_asso_alumni='non';
		}

		$reponse = $controler_signature($id_article,
			_request('session_prenom'),_request('session_nom'), _request('session_naissance'),_request('session_email'),$ecole_alumni, _request('session_pays'),$membre_asso_alumni,_request('session_linkedin'),
			_request('session_entreprise'),_request('session_fonction'), _request('session_pays_entreprise'),_request('session_ville'), $autorisation_infos_publiques,
...
function inc_controler_signature_dist($id_article, $prenom, $nom, $naissance, $mail, $ecole_alumni, $pays, $membre_asso_alumni, $linkedin, $entreprise, $fonction, $pays_entreprise, $ville, $autorisation_infos_publiques, $message, $site, $url_site, $url_page) {
	...
	$statut = "";
	if (!$ret = signature_a_confirmer($id_article, $url_page, $prenom, $nom, $naissance, $mail, $ecole_alumni, $pays, $membre_asso_alumni, $linkedin, $entreprise, $fonction, $pays_entreprise, $ville, $autorisation_infos_publiques, $site, $url_site, $message, 'fr'/*inutilise*/,
		$statut)
	) {
		return _T('petitions:form_pet_probleme_technique');
	}
	...
	if ($ecole_alumni = _request('session_ecole_ajout') and strlen($ecole_alumni) > 0) {
		$id_ecole = sql_insertq("spip_ecoles_alumni", array('nom' => $ecole_alumni, 'id_pays' => $pays, 'statut' => 'prop'));
	}
	else{
	...
	$id_gis = sql_insertq("spip_gis", array('titre' => _request('ville'), 'lat' => _request('lat'), 'lon' => _request('lon'), 'zoom' => _request('zoom'), 'pays' => _request('pays'), 'ville' => _request('ville')));
...
function signature_a_confirmer($id_article, $url_page, $prenom, $nom, $naissance, $mail, $ecole_alumni, $pays, $membre_asso_alumni, $linkedin, $entreprise, $fonction, $pays_entreprise, $ville, $autorisation_infos_publiques, $site, $url, $msg, $lang, &$statut) {
